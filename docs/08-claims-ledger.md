...
| C6 | Chemically valid ring/2-cell lifts improve molecular property prediction over the graph route | Differentiable liftings, TopoBench | OGBG-MOLHIV official scaffold split and evaluator, matched configs, multiple seeds | Graph route matches or beats the ring-lift route | **Strict form not supported; redesign direction validated** — v1 cell 0.723 ± 0.017 vs graph 0.771 ± 0.014; `molecular_mode` (strongest-bond max + ring size) recovers +0.034 to 0.757 ± 0.002 (most consistent route) but still trails by 0.014; v3 bond-type histograms are a negative ablation (0.729, scaffold-distribution noise) |
