...
| C3 | A cost-aware router specializes by structural regime | MoE routing, learned topological liftings | Regime-conditioned route analysis and fixed/random/oracle comparisons | Route collapse or shortcut behavior | **Supported with material seed variance (confirmatory, n=7)** — margin over best fixed +0.046, 95% CI [+0.010, +0.082] (t(6)); MI > 0 on all seeds (mean 0.052), utilization non-collapsed in 3/5 confirmatory seeds; one seed (s2) fails the utility criterion with partial collapse to cell — see `docs/17-gate2-confirmatory.md` |
